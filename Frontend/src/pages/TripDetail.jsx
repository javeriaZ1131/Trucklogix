...
import { useState, useEffect } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import { ArrowLeft, Map, FileText, List, ChevronDown, ChevronUp } from 'lucide-react'
import { fetchTrip } from '../api/tripService'
import RouteMap from '../components/RouteMap'
import ELDLogSheet from '../components/ELDLogSheet'
...
export default function TripDetail() {
  ...
  const [mapLoaded, setMapLoaded] = useState(false)
  const [expandedDay, setExpandedDay] = useState(null)
  const [tripData, setTripData] = useState(null)   // raw trip row
  ...
  // Build a trip shape that RouteMap + ELDLogSheet expect
  const tripForMap = {
    stops_summary: result.stopsSummary,
    routeCoords:   result.routeCoords,
    eldLogs:       result.eldLogs,
    driverName:    tripData.driver_name,
    carrierName:   tripData.carrier_name,
    truckNumber:   tripData.truck_number,
    trailerNumber: tripData.trailer_number,
  }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Trip #{tripData.id}</h1>
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusBadge[tripData.status] || statusBadge.Planned}`}>
              {tripData.status}
            </span>
          </div>
          <p className="text-gray-600 dark:text-gray-400">
            <span className="font-medium">{tripData.pickup_location}</span>
            <span className="mx-2 text-gray-400">to</span>
            <span className="font-medium">{tripData.dropoff_location}</span>
          </p>
        </div>
        <div className="flex gap-6 text-right">
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wider">Distance</p>
            <p className="text-xl font-bold text-gray-900 dark:text-white">{result.distance.toLocaleString()} mi</p>
          </div>
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wider">Duration</p>
            <p className="text-xl font-bold text-gray-900 dark:text-white">{result.duration}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Total Distance', value: `${result.distance.toLocaleString()} mi` },
          { label: 'Driving Time',   value: result.drivingTime },
          { label: 'Total Stops',    value: result.stops },
          { label: 'Fuel Stops',     value: result.fuelStops },
        ].map(({ label, value }) => (
          <div key={label} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 px-4 py-3">
      ...
      {tab === 'route' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden" style={{ minHeight: 380 }}>
            {mapLoaded
              ? <RouteMap trip={tripForMap} />
              : <div className="flex items-center justify-center h-full min-h-[320px] text-gray-400 text-sm">Loading map...</div>
            }
          </div>

          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
            <h3 className="font-semibold text-gray-900 dark:text-white mb-1">Stops Summary</h3>
            <p className="text-xs text-gray-400 mb-4">Scheduled points along the route.</p>
            <div className="space-y-4">
              {result.stopsSummary.map((stop, i) => {
                const col = STOP_COLORS[stop.type] || { dot: '#6b7280' }
                return (
                  <div key={i} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div className="w-3 h-3 rounded-full shrink-0 mt-0.5" style={{ background: col.dot }} />
                      {i < result.stopsSummary.length - 1 && (
                        <div className="w-px bg-gray-200 dark:bg-gray-700 mt-1" style={{ minHeight: 20 }} />
                      )}
                    </div>
                    <div className="pb-3 min-w-0">
                      <div className="flex justify-between gap-2">
                        <p className="font-medium text-sm text-gray-900 dark:text-white">{stop.location}</p>
                        {stop.miles > 0 && <span className="text-xs text-gray-400 shrink-0">{stop.miles.toLocaleString()} mi</span>}
                      </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between text-sm">
              ...
              <span className="font-bold text-gray-900 dark:text-white">{result.distance.toLocaleString()} mi</span>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'eld' && (
        <div className="space-y-5">
          {result.eldLogs.map(log => (
            <ELDLogSheet key={log.day} log={log} trip={tripForMap} />
          ))}
        </div>
      )}
          ...
          {result.eldLogs.map((log, di) => {
            ...
            return (
              <div key={di} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                >
                  <div className="text-left">
                    <p className="font-semibold text-gray-900 dark:text-white text-sm">Day {log.day}</p>
                    <p className="text-xs text-gray-400 mt-0.5">Driving: {log.drivingTime} · On Duty: {log.totalOnDuty}</p>
                  </div>
                  {isOpen ? <ChevronUp size={16} className="text-gray-400" /> : <ChevronDown size={16} className="text-gray-400" />}
                </button>
                {isOpen && (
                    ...
                    {log.entries.map((entry, ei) => (
                      <div key={ei} className="flex items-start gap-3 px-5 py-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-0.5">{entry.status} — {entry.location}</p>
                          {entry.notes && <p className="text-xs text-blue-500 italic mt-0.5">{entry.notes}</p>}
                        </div>
                      </div>
                    ))}
